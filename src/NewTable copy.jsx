...
function SimpleDataGrid2() {
    ...
    const generatePDF = () => {
        ...
        function addTableWithCheck(doc, tableHeader, tableData, title,  rowHeight = 8, footerHeight = 10) {
          // Calculate space remaining on the page
          const usedSpace = doc.lastAutoTable.finalY ; // Start from finalY if provided
          const spaceRemaining = doc.internal.pageSize.height - usedSpace - footerHeight; // Remaining space minus footer height
...
const table1Data = data.map((row) => [row.id, row.email, row.phone]);
addTableWithCheck(doc, table1Header, table1Data, 'Contact');

// Table 2
const table2Header = ['ID', 'University'];
const table2Data = data.map((row) => [row.id, row.address]);
